...
```{r}
child_trauma <- read.table("/Volumes/SanDisk/Data/CSV data/trauma.csv", sep = ",", header = TRUE)
...
```{r}
mod1 <- lm(depress~lonely*ptsd, data = child_trauma); summary(mod1)
mod2 <- lm(depress~lonely + ptsd + ptsd:lonely, data =child_trauma); summary(mod2)
```

**Is the interaction effect significant? What can we say about this relationship?**

b1: Depression is expected to decrease by .66 units when ptsd is 0 and loneliness increases by one unit. This effect is not significant, b1 = -0.66, t = -0.45, p = .65.

b2: Depression is expected to decrease by .26 units when loneliness is 0 and ptsd increases by one unit. This effect is significant, b2 = -0.26, t = -2.15, p = .03.
...
# assemble into final data frame
