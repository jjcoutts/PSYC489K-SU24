...
This weeks session is about catching everyone up to speed on R and regression. Struggles to be up
...
```{r}
# createvector with the INTEGERS 1 through 100 
...
```{r}
# loop through the COLUMNS of the data frame above and 
...
```{r}
# read in the dataset "austress" from R using the data() command. 
...
char = c("a","b",c,"D")
Print(char)
df = is.data.frame(numbers = 1:3, lettes = c("A","B","C"))
...
```{r}
craving <- read.table(file= "", sep= "", header =)
```
